...
static double lastBest = DBL_MAX;
static int noImproveGen = 0;
    ...
    while (_FessNow < _FESS)
    {
        ...
        for (int i = 0; i < _NP; i++)
        {
            ...
            if (_X[i]._fitness >= _U._fitness)
            {
                ...
                // // ILS
                // if (_X[i]._fitness < _Best && tool.rand_double(0, 1) < 0.1)
                //     ApplyILS(_X[i]);
            }
        }
        ...
        // ILS
        // 每隔固定世代對目前最佳個體進行 ILS（穩定且低干擾）
        if (_FessNow % (_NP * 20) == 0)  // 每 20 世代
        {
            int bestIndex = 0;
            for (int i = 1; i < _NP; ++i) {
                if (_X[i]._fitness < _X[bestIndex]._fitness)
                    bestIndex = i;
            }
            ApplyILS(_X[bestIndex]);
        }
        ...
        // ---------------------- Adaptive Conditional ILS Start ----------------------
        if (_Best < lastBest * 0.999) {  // 有改善（幅度大於0.1%）
            noImproveGen = 0;
            lastBest = _Best;
        } else {
            noImproveGen++;
        ...
        // 每 20 代且停滯 >= 3 次才觸發 ILS
        if ((_FessNow % (_NP * 20) == 0) && (noImproveGen >= 3)) {
            int bestIndex = 0;
            for (int i = 1; i < _NP; ++i) {
                if (_X[i]._fitness < _X[bestIndex]._fitness)
                    bestIndex = i;
            ...
            // 動態調整 ILS 擾動強度：early strong, late weak
            double progressRatio = (double)_FessNow / _FESS;
            double scale = 0.001 * (1.0 - progressRatio);  // early: 0.001, late: close to 0

            for (int j = 0; j < _Dim; ++j) {
                double perturb = tool.rand_normal(0, scale);
                _X[bestIndex]._position[j] += perturb;

                if (_X[bestIndex]._position[j] < _lowerBound)
                    _X[bestIndex]._position[j] = _lowerBound;
                else if (_X[bestIndex]._position[j] > _upperBound)
                    _X[bestIndex]._position[j] = _upperBound;
            }

            double new_fit = calculate_test_function(&_X[bestIndex]._position[0], _Dim, _Func);
            _FessNow++;
            if (new_fit < _X[bestIndex]._fitness)
                _X[bestIndex]._fitness = new_fit;
        }
        // ---------------------- Adaptive Conditional ILS End -------------------------
...
void Shade::ApplyILS(_Particle &p)
{
    for (int i = 0; i < _Dim; i++)
    {
        // double perturb = tool.rand_normal(0, 0.01); // 微擾
        double perturb = tool.rand_normal(0, 0.0005); // 微擾
        p._position[i] += perturb;
        if (p._position[i] < _lowerBound)
            p._position[i] = _lowerBound;
        else if (p._position[i] > _upperBound)
            p._position[i] = _upperBound;
    }
    double new_fit = calculate_test_function(&p._position[0], _Dim, _Func);
    _FessNow++;
    if (new_fit < p._fitness)
        p._fitness = new_fit;
}
